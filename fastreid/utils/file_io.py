...
def get_cache_dir(cache_dir: Optional[str] = None) -> str:
    ...
    if cache_dir is None:
        # Use platform-appropriate default cache directory
        if os.name == 'nt':  # Windows
            default_cache = os.path.join("~", "AppData", "Local", "torch", "fvcore_cache")
        else:  # Unix-like systems
            default_cache = "~/.torch/fvcore_cache"
        cache_dir = os.path.expanduser(
            os.getenv("FVCORE_CACHE", default_cache)
        )
    return cache_dir
